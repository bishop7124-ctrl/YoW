...
**Merging (standing authorization, given by the user 2026-09-09):** once a PR has passed `code-reviewer` (+ `security-reviewer` if warranted), `qa-engineer` has verified it, and CI is fully green (not pending, not red) — merge it yourself: `gh pr merge <n> --squash --delete-branch`, or `--auto` if CI is still running and you don't want to wait. Don't ask first; this is a "how," not a "whether," now. Do **not** auto-merge, and instead flag it and move on: a PR whose CI is red or still pending with no other work to do while it finishes, a PR with unresolved review comments, a PR you can't get conflict-free against `main` without a judgment call you're not confident in (see the conflict-resolution note below), or anything touching Stripe live-mode config, RLS policies, or auth in a way `security-reviewer` flagged as needing a human look. If a PR sat open long enough that `main` moved past it, rebase/merge `main` into it yourself in a scratch worktree (never the shared main checkout), resolve conflicts using the same judgment `code-reviewer` would (prefer whichever side is more complete/more recently verified when both sides changed the same roadmap-doc row; never silently drop a real code change), re-run the relevant tests, then push and merge — don't leave it to rot a second time.

**Still never**, without current, explicit authorization for that specific action: flip a production/live-payment toggle, apply a database migration against production, rotate/set a live secret, or take any other outward-facing/hard-to-reverse action outside normal code merges. Standing authority covers driving implementation work all the way to merged-on-`main`; it does not cover deployment, production data changes, or payment-system state, unless the user has separately said otherwise for that specific action.
