...
export async function downgradeToFreeLocally(supabaseAdmin, user) {
  ...
  if (wasMonthly) {
    // Mirrors the real cancellation path in stripe-webhook.js: mark that this
    // account was previously a monthly subscriber so the free tier locks the
    // active project selection the same way a real cancellation would.
    const existingUser = user.user_metadata || {}
    attributes.user_metadata = { ...existingUser, was_monthly: true }
  }

  await supabaseAdmin.auth.admin.updateUserById(user.id, attributes)
