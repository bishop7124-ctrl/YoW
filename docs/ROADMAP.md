...
| Vault metadata | `nf_localWriteAt`, `nf_localOwner`, `nf_storageMode:<userId>`, `nf_localFirstSnapshot:<userId>` | `vault.db` |
| Large binary project data | IndexedDB `yow-maps` pixel data (`mapStorage.js`) | Vault blob storage (Phase 3) |
...
| Desktop Lifetime app and local vault | Lifetime is fulfilled by a downloadable desktop app (macOS + Windows) storing projects in a local on-device vault, with optional account-based Cloud Sync, licence activation with offline grace, and no reliance on browser storage. Free/Monthly remain browser/cloud plans. Full spec in [PRD: Desktop Lifetime App and Local Vault](#prd-desktop-lifetime-app-and-local-vault). | Phase 1 complete 2026-07-04 | Phase 2: Tauri shell proof of concept (webview QA spike on manuscript/map/exports, unsigned local builds, go/no-go record). Storage regression QA tracked in [docs/QA_PLAN.md](QA_PLAN.md) Priority 1. | Product/Engineering |
