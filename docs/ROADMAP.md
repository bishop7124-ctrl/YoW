...
| 2026-07-19 QA: export package expectations are unclear and partly wrong | Export ownership risk. Novel ZIP backups included comic pages/panels, while the manual expectation was that backup ZIPs remain YOW JSON for restore and readable Word documents should include worldbuilding/project data. | Confirmed, launch blocker until product expectation is resolved | Scope backup ZIP contents by project type, then decide whether the launch promise includes a readable all-project/worldbuilding DOCX bundle or whether World Bible PDF/HTML is the readable export path; update UI/checklist copy accordingly and QA small/medium/large exports. | Do not remove JSON backup files; they are needed for restore. |
