...
| 7. Launch | Large project performance | Dashboard, manuscript, search, and export remain usable on a realistic large novel project. | Yes | Needs QA | 2026-07-20: added `scripts/seed-stress-test.mjs`, a local-only generator (no live-account writes) that builds a project matching the checklist spec — 5 acts, 20 chapters, 80 scenes, exactly 80,000 words, 50 characters, 30 locations, 40 lore entries, 25 timeline events — as a localStorage-shaped JSON payload. Data generation confirmed exact; loading it into a running app and measuring dashboard/sidebar/search/export behaviour is still outstanding, blocked this session by the local preview browser's localhost tunnel being unreachable. Create/load the stress project and measure obvious failures next session. | Engineering/QA |
...
| 2026-07-19 QA: AI tools need provider/error/progress UX | AI feature quality issue. Plot Hole, Lore Conflict, and Style tools run too long on a 40k-word project without clear progress/error reporting; no-provider states are missing in some AI tools; findings do not link users to the area to fix. | Confirmed | Add progress/status UI, timeouts or chunking for long manuscripts, consistent no-provider messages, actionable finding links, and visible AI tools across all project types where supported. | Character Interview passed. AI Import without a configured key also needs review because the import appeared to work unexpectedly. |
| 2026-07-19 QA: public page routing and metadata gaps | Marketing/SEO quality issue. `/ai-overview` failed; FAQ header missed Founders; mobile marketing nav lacks hamburger/collapsed navigation; OG/Twitter tags were missing; public info pages need real screenshots for map/timeline/worldbuilding. | Mobile nav fixed 2026-07-20; rest still open | Fix route/header/nav metadata issues and add product screenshots to public info pages. Re-run marketing page pass across desktop/tablet/mobile. 2026-07-20: found and fixed a real gap — `.marketing-nav-links` was `display:none` at 421-900px viewports (covers the 768px QA checkpoint) with no alternative at all; a narrower ≤420px rule brought links back as a compact strip, so the 421-900px band was fully broken. Added a real hamburger menu to `MarketingNav.jsx` with a collapsible panel (links + auth actions), replacing both old CSS hacks with one unified ≤900px treatment. Live-verified at 768px and 375px. `/ai-overview`, FAQ header Founders link, and OG/Twitter tags remain open. | Homepage/features/about/beta-disclaimer and most static pages loaded. |
