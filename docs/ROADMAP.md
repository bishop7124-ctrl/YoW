...
| `tourDefinitions.js` | Step arrays for Library, Manuscript, Characters, Locations, Lore, Ideas, Map, AI Tools, and Timeline tours. Ready to wire into each section. | Definitions complete, section wiring pending |

Integration:
- `App.jsx`: imports `useTourStore`, passes `tourStore` to `NovelManager`, shows `WelcomeWizard` when `!wizardShown && novels.length === 0`.
- `NovelManager.jsx`: renders checklist, `?` tour button in top bar, `data-tour` attributes on key elements, library `OnboardingTour`.
- Section tours (Manuscript, Characters, etc.) use the same `OnboardingTour` engine — wire in using the `data-tour` attribute pattern and `tourDefinitions.js` step arrays.

QA needed: wizard create flow on a real fresh account; checklist milestone accuracy; tour spotlight positioning at mobile widths.
