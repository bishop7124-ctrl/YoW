...
- [x] Unsigned Windows installer build path (2026-07-15, zero-cost decision): product decision to build the free Windows path now rather than wait for paid signing, since Windows can launch unsigned via a visible SmartScreen bypass while macOS has no free equivalent. Added `.github/workflows/desktop-build-windows.yml` (manual `workflow_dispatch`, builds on GitHub's free `windows-latest` runner, no local Windows machine needed) producing an unsigned NSIS installer via the new `desktop:build:nsis` script; `src-tauri/tauri.conf.json` now sets explicit `bundle.windows.webviewInstallMode: downloadBootstrapper` so the installer bootstraps WebView2 when not already present. `/download` page gained matching first-launch SmartScreen instructions (More info → Run anyway) alongside the existing macOS instructions. Remaining: run the workflow, manually attach the resulting `.exe` to a GitHub release the same way the macOS beta zip was published, then set `DESKTOP_DOWNLOAD_URL_WINDOWS` in Vercel. macOS stays exactly as-is (unsigned zip, testers-only) until the Apple Developer Program is paid for.
- [x] Tauri updater wired to a free release channel; minisign keys generated (2026-07-15). This uses Tauri's own update-integrity signing (a project-owned minisign keypair), which is unrelated to and free regardless of OS-level code signing/notarization — it proves an update came from this project's release process, not that Apple/Microsoft vouch for the binary. Public key committed in `src-tauri/tauri.conf.json` (`plugins.updater.pubkey`); endpoint set to `https://github.com/bishop7124-ctrl/StoryAtlas/releases/latest/download/latest.json` (GitHub Releases as a free hosting channel — always resolves to the newest published release's copy of that filename). Added `tauri-plugin-updater` + `tauri-plugin-process` (Rust) and `@tauri-apps/plugin-updater` + `@tauri-apps/plugin-process` (frontend); `updater:default` and `process:allow-restart` capability permissions added. `src/utils/desktopUpdater.js` checks for updates on desktop startup only (`isDesktopAppRuntime()` gated); `App.jsx` shows a dismissible "Update available" toast (same `membership-toast` pattern as the licence/device-limit toasts) with a "Restart to update" action that downloads, installs, and relaunches. `.github/workflows/desktop-build-windows.yml` now signs the build for updater purposes when `TAURI_UPDATER_PRIVATE_KEY`/`TAURI_UPDATER_PRIVATE_KEY_PASSWORD` repo secrets are present (harmless no-op until they're added) and uploads the resulting `.sig` alongside the `.exe`. `scripts/generate-updater-manifest.mjs` builds the `latest.json` manifest from a version, notes, and per-platform `url=sigPath` pairs, for uploading alongside installers on a GitHub release. **User action required:** the generated private key + password live only in this session's scratchpad (ephemeral) — move them to a password manager immediately, then add them as the `TAURI_UPDATER_PRIVATE_KEY` and `TAURI_UPDATER_PRIVATE_KEY_PASSWORD` repo secrets in GitHub before the next Windows CI build if signed updates are wanted. Losing this key means a new keypair (and a new `pubkey` in `tauri.conf.json`) is needed, breaking the update chain for anyone already on the old key. Remaining: publish a release with `latest.json` attached and verify a real update round-trip once two Windows builds exist.
