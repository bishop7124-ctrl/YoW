...
| Reliable map builder | D&D/TTRPG and worldbuilding users can create reliable, readable maps for planning. The map builder must feel predictable, persist safely, export/restore correctly, and include a grid overlay for movement/planning. It remains a planning map tool, not a live virtual tabletop. | Complete clean-slate remake 2026-06-27; needs authenticated browser QA | Current map builder has multi-map support, typed layers, paths, terrain ridges, stamps, labels, linked locations, JSON import/export, grouping, undo/redo, rendered export plates, high-resolution PNG export, command-bar selected-object deletion, and per-object layer-row deletion. **V2 PRD additions (2026-06-27):** Note tool added to all map types — click places a sticky-note marker with title, body, private/public visibility, and GM-only flag for campaign projects; notes render as a distinctive yellow card icon on the canvas. Campaign Sketch and Blueprint added as style presets; Campaign Sketch renders a cream paper with faint ruled lines; Blueprint renders a dark blue technical-drawing canvas with a subtle grid. Notes field (textarea) added to the Object Properties inspector for every object type so any map element can carry private notes. GM-only toggle appears in the note modal and inspector for D&D/TTRPG campaign projects. **Stability follow-up (2026-06-27):** New Map now uses one shared typed create dialog from empty state, editor top bar, and Map settings; selected map type and style persist at creation; default map type fallback corrected to `region`; stamp picker renders the actual PNG stamp previews with intentional fallbacks for missing art; layer z-order buttons and drag sorting now rewrite a deterministic top-to-bottom draw order; the map workspace no longer reserves the blank AI footer strip. **Terrain texture polish (2026-06-28):** Terrain fills now share dense clustered, staggered, polygon-safe illustrated placement across forest, mountains, hills, desert, swamp, farmland, tundra/snow, volcanic, and grassland styles, with repeated terrain marks omitted rather than clipped when they would cross terrain boundaries, while farmland keeps continuous crop-row linework; terrain background opacity is independent from the symbols and can be set to 0% or Transparent bg for truly transparent terrain fills with no terrain-specific wash; unselected terrain regions no longer draw dotted/dashed borders. Hills now render as fewer rounded humps with reserved footprints so hill symbols do not overlap. Desert now uses quieter sparse low-opacity wind-dune arcs so it reads as a subtle texture rather than a loud pattern. Farmland now uses coherent parallel crop-row bands and light divider strokes instead of random square field blocks. Tundra and Snow/Ice are merged into one Snowy Tundra terrain type; legacy snow objects render the same high-contrast snowdrift hatches, angular ice shards, and darker crack clusters. Swamp now renders in the same drawn terrain language as the rest of the terrain set, using a muted muddy-water wash, cattail reed clusters, lily pads, ripple marks, and fine speckled marsh texture instead of ambiguous reed-stick marks, polka dots, blobs, or perfect ovals. All terrain types now expose per-object Symbol size controls for scaling drawn marks and spacing. Forest and mountains keep their bespoke polish: smaller, more densely populated irregular canopy clumps with muted olive-gray painted canopy/trunk fills instead of solid silhouettes or triangle piles, continuous low canopy texture to reduce blank gaps, undergrowth, forest border-safe placement that omits any tree whose footprint would cross the terrain boundary, closer alternating mountain rows, mixed one- and two-peak mountain clusters, wider jagged-edged alpine ridges, solid body fills under fully opaque snow/shaded faces and detail strokes, opaque left faces, row-depth-only foothill curves, front-to-back mountain ordering by vertical position, and border-safe mountain placement that omits any cluster whose footprint would cross the terrain boundary while preserving existing map data. **Polygon drawing follow-up (2026-06-28):** Landmass, Terrain, and Region polygon tools now offer Point and Freehand draw modes; parchment maps default new landmasses to a light parchment-toned fill instead of dark green. **Geometry edit follow-up (2026-06-28):** Selected geometry objects can switch between Points mode for vertex-by-vertex editing and Resize mode for whole-shape corner scaling. **Point-edit follow-up (2026-06-28):** Selected landmasses, terrain regions, political regions, rivers, roads, borders, and legacy mountain-line paths now expose draggable point handles with snap-to-grid and undo support. Next action is authenticated browser QA for geometry edit-mode switching, point-by-point editing, whole-shape resize handles/scaling, polygon Point/Freehand modes on landmass/terrain/region tools, parchment landmass default color, point-handle dragging/persistence/undo, note placement/persistence/export, New Map type/style creation paths, stamp preview/placement matching, no dotted/dashed border on unselected terrain, terrain background transparency at 0% or Transparent bg without fading symbols or leaving terrain-specific wash shadows, terrain visual density in canvas and exports across all terrain types, confirming repeated terrain marks are omitted rather than clipped at terrain borders except for continuous farmland crop rows, confirming desert stays subtle with sparse low-opacity dune arcs, confirming farmland reads as coherent parallel crop-row bands rather than random square plots, confirming Snowy Tundra is the single cold terrain type and reads with high-contrast snowdrift hatches, angular ice shards, and darker crack clusters, confirming swamp terrain matches the drawn look of the other terrain types while reading as one integrated wetland with muddy water, reeds, lily pads, ripple marks, and speckled marsh texture rather than clipart, unclear sticks, polka dots, blobs, or perfect little ovals, hill terrain has fewer non-overlapping rounded humps, terrain Symbol size works across all terrain types, forest trees are densely populated, muted green, not clipped at terrain borders, do not read as boulders, solid black stamps, or a mass of triangles, have no major blank gaps, and mountain peaks follow staggered alternating rows with irregular overlapping hand-drawn alpine clusters instead of fuzzy extra marks, transparency bleed-through, clipped border fragments, visible non-front-row mountain bottoms, solid clipart silhouettes, or perfect triangle shapes, layer drag and arrow ordering, bottom-strip removal, and all prior map builder checks. | Product/Design/Engineering |
