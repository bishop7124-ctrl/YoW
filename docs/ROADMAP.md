...
| Log in again after sign-out opens homepage | Logout redirect adds an extra step by returning users to the marketing homepage instead of the login form. | Fixed | The signed-out interstitial now opens `/login` directly when users choose Log in again; Go to homepage remains unchanged. Include direct auth URLs in the auth regression pass. | Engineering/QA |
