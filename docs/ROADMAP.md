...
Launch triage decision (2026-07-04): feature-freeze the Map Builder for new depth until the full QA checklist passes or specific defects require small corrective changes. Schedule polish is accepted into launch scope as a practical configurable story/campaign calendar, but still not marketed as a full invented-calendar engine. Local-first mode is accepted into launch scope as a data-safety and trust feature; collaboration remains excluded.
...
> Scope update 2026-07-04: the Lifetime tier is being re-architected as a downloadable desktop app with a local project vault — see [PRD: Desktop Lifetime App and Local Vault](#prd-desktop-lifetime-app-and-local-vault). The entitlement rules, grace/archive policy, storage caps, and renewal billing below remain in force; what changes is the fulfilment mechanism: Lifetime local storage moves from browser Local Mode to the desktop vault. Browser Local Mode remains the transitional fallback until the desktop app ships.
...
This supersedes browser Local Mode as the Lifetime fulfilment mechanism. The browser Local-first mode (see "User-controlled Local-first mode") remains a web-app data-safety feature and the transitional fallback until the desktop app ships, but it is no longer what the Lifetime tier sells. The desktop app removes ongoing hosting cost for Lifetime users: local writing generates zero Supabase traffic.
...
| Cloud sync | `src/utils/firestoreSync.js` (misnamed — it is Supabase) — per-entity table upserts, debounced from `useStore` effects; master switch is the `cloudSyncEnabled` option `App.jsx` passes into `useStore` | Unchanged for browser plans. On desktop it becomes the optional Cloud Sync bridge, off by default unless the add-on is entitled and enabled |
...
| Local-first mode | `src/utils/storageMode.js` — per-user mode preference + snapshot in localStorage | Kept for the web app; its cloud-pause semantics are reused by the desktop sync bridge |
...
Status: Phase 1 started 2026-07-04.
...
- [ ] Migrate remaining project-content localStorage users onto the abstraction: scene drafts (`manuscriptUtils.js`), scene versions (`sceneVersions.js`), map JSON helpers. (UI preferences — theme, tours, AI settings — can migrate later; they are not part of the data-safety promise.)
- [ ] Decide and document which `nf_*` keys are vault data vs device preferences.

**Phase 2 — Desktop shell proof of concept**

- [ ] `src-tauri/` scaffold wrapping the existing Vite build; app boots to the current UI on macOS and Windows.
- [ ] Webview QA spike: manuscript editor + caret comfort, map canvas (draw/undo/PNG export), ZIP/DOCX/PDF blob downloads via native save dialog, Supabase auth flow in the webview.
- [ ] Prove unsigned local builds for macOS (arm64) and Windows (x64); document the signing/notarization pipeline requirements.
- [ ] Go/no-go on Tauri vs Electron recorded here.

**Phase 3 — Local project vault and auto-save**

- [ ] `desktopVaultBackend` implementing the Phase 1 contract: in-memory mirror hydrated from SQLite at startup, async write-behind to `vault.db` (WAL), flush on close.
- [ ] Vault folder creation, locate/relocate UI, and first-run vault setup.
- [ ] Import ZIP into vault (web → desktop migration path) and export ZIP from vault.
- [ ] Rolling snapshot writer to `YOW/Backups/` + snapshot restore flow.
- [ ] Startup integrity check + recovery from snapshot/`vault.db.bak`.
- [ ] Map pixel data storage in the vault (replaces IndexedDB on desktop).
...
- [ ] Reuse `firestoreSync.js` upsert/load against vault data; sync toggle per device; entitlement-gated.
- [ ] Conflict rule: vault is canonical on desktop; cloud pull never overwrites newer local rows (reuse the `_savedAt`/local-write-marker freshness comparison); explicit conflict prompt where timestamps disagree materially.
- [ ] Sync status UI (last synced, paused, error) and the lapsed-hosting behavior from the Lifetime Local Mode PRD.
- [ ] Web ↔ desktop round-trip QA: edit on desktop, sync, open in browser Monthly-style session on the same account, and back.

**Phase 6 — Installer, build, release QA**

- [ ] Signed + notarized macOS DMG; signed Windows installer (NSIS/MSI) with WebView2 bootstrap.
- [ ] Tauri updater wired to account-tied release channel; minisign keys in CI secrets.
- [ ] Download delivery from the website gated on Lifetime/Founder entitlement.
...
- [ ] Pricing/FAQ/Terms copy updated to the desktop Lifetime model (only at this phase — public copy must not advertise the desktop app before it is real).
...
| Configurable Schedule calendar | Schedule remains the launch story/campaign calendar, but it should support custom month labels, configurable month count/lengths, configurable week length/day labels, and project-specific category labels. It must persist, export/restore, and remain honest as Schedule rather than a full invented-calendar engine. | Accepted into launch scope 2026-07-04; not implemented | Extend Schedule settings with month/week structure and labels; migrate existing 12×30 schedules safely; update schedule UI, imports/exports, and QA. Exclude leap rules, moons, date conversion engines, and complex recurrence unless separately re-decided. | Product/Engineering/QA |
| Desktop Lifetime app and local vault | Lifetime is fulfilled by a downloadable desktop app (macOS + Windows) storing projects in a local on-device vault, with optional account-based Cloud Sync, licence activation with offline grace, and no reliance on browser storage. Free/Monthly remain browser/cloud plans. Full spec in [PRD: Desktop Lifetime App and Local Vault](#prd-desktop-lifetime-app-and-local-vault). | Phase 1 (storage abstraction) started 2026-07-04 | Complete Phase 1 remaining items (scene drafts/versions/map helpers onto the abstraction, key classification), then Phase 2 Tauri proof of concept. QA checks tracked in [docs/QA_PLAN.md](QA_PLAN.md) Priority 1. | Product/Engineering |
| User-controlled Local-first mode | Users can choose Local-first writing even while their account has Cloud Sync entitlement. Local-first mode stores and edits from this device first, pauses automatic cloud writes and remote imports, shows a visible status, and never overwrites local work with older cloud data. Re-enabling Cloud Sync uploads the current browser copy unless a later conflict UI offers an explicit alternative. | First safe slice implemented 2026-07-04; needs hardening + QA | Account Settings > Membership now exposes a Local-first writing toggle; the app stores a per-user storage-mode preference, disables automatic Firestore sync while Local-first is active, skips remote load while active, and resumes cloud sync from the current browser copy when re-enabled. Remaining work: automatic local snapshots, IndexedDB-backed project storage for larger projects, explicit cloud backup/restore controls, conflict preview, status copy across workspace, export prompts, and full offline/account-switch QA. | Product/Engineering/QA |
...
| Feedback endpoint hardening | `/api/submit-feedback` is unauthenticated with no rate limiting or input length caps, so it can be scripted to flood the owner inbox through the configured Gmail account. `nodemailer` ≤9.0.0 also carries one high-severity npm advisory (GHSA-p6gq-j5cr-w38f, `raw`-option file read/SSRF); the `raw` option is not used so practical exploitability is low. Found in the 2026-07-04 status review ([docs/STATUS_REPORT_2026-07-04.md](STATUS_REPORT_2026-07-04.md)). | Open | Add max lengths on title/message, per-IP rate limiting, and upgrade nodemailer to ≥9.0.3; optionally restrict CORS on API routes to `SITE_URL`. |
...
| --- | --- | --- | --- | --- |
| QA CI red since 2026-06-30 — lint errors committed to main | The GitHub Actions QA workflow failed on every push and scheduled run since 2026-06-30 because 11 `no-unused-vars` lint errors (dead terrain/swamp/tundra helpers in `src/components/Map/mapDraw.js`) blocked `npm run qa` at the lint step, so tests/build/load-check were not being exercised in CI. | Fixed locally 2026-07-04 — 20 dead functions removed; `npm run qa` green (0 errors, 47/47 tests, build + load check pass) | Commit and push, then confirm the next CI run passes. | Engineering |
