...
Status: Phase 3 local vault bridge in progress 2026-07-04.
...
- [ ] Prove unsigned local builds for macOS (arm64) and Windows (x64); document the signing/notarization pipeline requirements. 2026-07-04 macOS arm64 app-only release artifact builds locally: `YOW.app` (~15 MB), with manual ad-hoc wrapper signing verified by `codesign`. Tauri's canonical Finder-styled DMG still fails in this Codex/macOS environment because `hdiutil create`/attach/resize return `Device not configured`; a local fallback `makehybrid` package command now produces `YOW_0.1.0_aarch64.local.dmg` for Phase 2 proof. Final DMG mount/install verification still needs a normal macOS session or CI runner, and Windows x64 artifact still required.
...
- [ ] Vault locate/relocate UI and explicit first-run vault setup.
...
| Desktop Lifetime app and local vault | Lifetime is fulfilled by a downloadable desktop app (macOS + Windows) storing projects in a local on-device vault, with optional account-based Cloud Sync, licence activation with offline grace, and no reliance on browser storage. Free/Monthly remain browser/cloud plans. Full spec in [PRD: Desktop Lifetime App and Local Vault](#prd-desktop-lifetime-app-and-local-vault). | Phase 3 in progress; SQLite vault bridge added 2026-07-04 | Finish Phase 2 webview QA outside the current Codex sandbox or with native window-control access: focused-writing caret comfort, map draw/undo/PNG export, ZIP/DOCX/PDF save behavior, Supabase auth in WKWebView, Windows WebView2 boot/build parity, and final Tauri go/no-go. Continue Phase 3 with vault folder setup/relocation UI, ZIP import/export against the vault, snapshot/recovery, map pixel data migration, and native quit/reopen/force-quit verification. Storage regression QA tracked in [docs/QA_PLAN.md](QA_PLAN.md) Priority 1. | Product/Engineering |
