...
- Features: full app, unlimited projects, AI/advanced features, revocable after beta.
...
These say Lifetime/Founder unlock the desktop product. `DownloadPage` itself currently checks only `isLifetime`, excluding Beta even though membership says Beta is desktop-entitled. Platform download text includes unsigned-install workarounds. Do not ship these instructions as paid-product onboarding.
