...
function MapObject({ object: o, palette: p, selected, onPick, organicBorders, organicStrength, preview, draft }) {
...
export default function AtlasCanvas({ objects, metadata = {}, name, selectedId, onPick, svgRef, children, placementPreview, draftId, ...events }) {
  ...
  return <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} aria-label={name ? `${name} map canvas` : 'Map preview'} {...events}>
    ...
    {objects.filter(o => o.visible !== false).map(o => <MapObject key={o.id} object={o} palette={p} selected={o.id === selectedId} onPick={onPick} draft={o.id === draftId} organicBorders={metadata.organicBorders !== false} organicStrength={metadata.organicStrength || 12}/>)}
