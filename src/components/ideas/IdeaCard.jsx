...
function IdeaCard({ idea, isDragging, isDropBefore, onEdit, onDelete, onArchive, onRestore, onFavourite, onConvert, onAiExpand, onPointerDown, aiExpandId, readOnly, dragEnabled }) {
  return (
    <article data-card-id={idea.id} onClick={event => { if (!event.target.closest('button')) onEdit(idea.id) }} className="rounded-xl border bg-[var(--bg-main)] p-3 space-y-2 flex-shrink-0"
      ...
      <div className="flex items-start gap-2">
        ...
        <button type="button" className="flex-1 min-w-0 text-left text-sm font-semibold break-words" onClick={() => onEdit(idea.id)}>{idea.title || 'Untitled idea'}</button>
      ...
      {idea.description && <p className="text-xs text-[var(--text-muted)] whitespace-pre-wrap break-words line-clamp-4">{idea.description}</p>}
