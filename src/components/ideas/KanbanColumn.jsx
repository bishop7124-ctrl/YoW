  ...
  return (
    <section aria-label={status.label} className="flex flex-col h-full min-w-[280px] max-w-[320px] flex-[1_1_280px] rounded-2xl border bg-[var(--bg-nav)] overflow-hidden"
      ...
      <header className="px-3 py-2 border-b border-[var(--border)]">
        ...
        <p className="text-[10px] text-[var(--text-muted)] mt-0.5">{status.desc}</p>
      </header>
      <div data-column={status.id} data-column-body={status.id} className="flex-1 min-h-[120px] overflow-y-auto p-2.5 flex flex-col gap-2">
        ...
        {!cardProps.readOnly ? <button type="button" onClick={() => onEmptyClick(status.id)} className="rounded-lg border border-dashed border-[var(--border)] p-2 text-xs text-[var(--text-muted)]" aria-label={`Add idea to ${status.label}`}>Click to add an idea</button>
