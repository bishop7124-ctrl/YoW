...
export default function CharacterJourney({ character, characters, timeline, chapters, scenes, onSave, readOnly = false, onEditingChange }) {
  const journey = useMemo(() => normalizeJourney(character.journey), [character.journey])
  const hasContent = useMemo(() => hasJourneyContent(character.journey), [character.journey])
  const [showJourneyForm, setShowJourneyForm] = useState(false)
  const [beatTarget, setBeatTarget] = useState(undefined)
  const [expanded, setExpanded] = useState(() => new Set())
  const [error, setError] = useState('')
  const editing = showJourneyForm || beatTarget !== undefined
  useEffect(() => {
    onEditingChange?.(editing)
    return () => onEditingChange?.(false)
  }, [editing, onEditingChange])
  const indexes = useMemo(() => [timeline, chapters, scenes, characters].map(items => new Map(items.map(item => [item.id, item]))), [timeline, chapters, scenes, characters])

  const saveJourney = value => {
    if (readOnly || !onSave({ journey: { ...normalizeJourney(value), updatedAt: new Date().toISOString(), createdAt: value.createdAt || new Date().toISOString() } })) {
      setError('The journey could not be saved. Your changes have not been applied.')
      return false
    }
    setError('')
  ...
  const saveBeat = beat => {
    if (!saveJourney(upsertJourneyBeat(journey, beat))) return false
  ...
  const lookup = (index, id, fallback) => index.get(id)?.title || index.get(id)?.name || fallback
    ...
    return (
      <div className="lg:col-span-2">
        <StudioEmpty
          ...
          action={<StudioButton tone="primary" className="mt-4" disabled={readOnly || editing} onClick={() => setShowJourneyForm(true)}>Begin the journey</StudioButton>}
        />
    ...
    <div className="lg:col-span-2 space-y-6">
      ...
      <StudioNote>
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          ...
          <StudioButton tone="secondary" size="sm" disabled={readOnly || editing} onClick={() => setShowJourneyForm(true)}>Edit overview</StudioButton>
        </div>
      ...
      <StudioNote>
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <StudioButton tone="primary" size="sm" disabled={readOnly || editing} onClick={() => setBeatTarget(null)}>Add beat</StudioButton>
        </div>
        {journey.beats.length === 0 ? (
          ...
          <ol className="relative ml-3 border-l border-[var(--accent)]/35 pl-6">
            {journey.beats.map((beat, index) => {
              ...
              const links = [
                beat.timelineEventId && `Timeline: ${lookup(indexes[0], beat.timelineEventId, 'Missing event')}`,
                beat.chapterId && `Chapter: ${lookup(indexes[1], beat.chapterId, 'Missing chapter')}`,
                beat.sceneId && `Scene: ${lookup(indexes[2], beat.sceneId, 'Missing scene')}`,
                beat.linkedCharacterId && `Character: ${lookup(indexes[3], beat.linkedCharacterId, 'Missing character')}`,
              ].filter(Boolean)
              return (
                  ...
                  <article className={`rounded-xl border p-4 ${beat.isMajorTurningPoint ? 'border-[var(--accent)]/55 bg-[var(--accent-fade)]/30' : 'border-[var(--border)]'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <div className="flex gap-1">
                        <button type="button" aria-label="Move beat earlier" disabled={readOnly || editing || index === 0} onClick={() => moveBeat(beat.id, -1)} className="chip disabled:opacity-30">↑</button>
                        <button type="button" aria-label="Move beat later" disabled={readOnly || editing || index === journey.beats.length - 1} onClick={() => moveBeat(beat.id, 1)} className="chip disabled:opacity-30">↓</button>
                        <button type="button" disabled={readOnly || editing} onClick={() => setBeatTarget(beat)} className="chip">Edit</button>
                        <button type="button" disabled={readOnly || editing} onClick={() => deleteBeat(beat.id)} className="chip">Delete</button>
                      </div>
                    </div>
                    {isOpen && (
