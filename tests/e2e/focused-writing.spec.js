...
//
//  1. Entering focus mode is the topbar's visible "Focus" control with the
//     accessible name "Enter focused writing mode"; the exit control is named
//     "Exit focused writing mode".
...
const waitForCaretSettled = async (page) => {
  await expect.poll(async () => page.evaluate(({ threshold, margin }) => {
    ...
    const lineBottom = textarea.getBoundingClientRect().bottom - (Number.parseFloat(style.paddingBottom) || 0)
    const containerRect = container.getBoundingClientRect()
    const marginBelow = (containerRect.top + container.clientHeight) - lineBottom
    const marginAbove = lineBottom - lineHeight - containerRect.top
    return marginBelow >= margin && marginAbove >= 0
...
test('the caret returns to its comfort position after a typing burst', async ({ page }) => {
  ...
  // The correction must bring the caret back to a comfortable visible position.
  // Exact pixel parity with the starting position is intentionally not asserted:
  // the focused shell can settle at a different comfortable offset after the
  // topbar hides/reappears during typing.
  await waitForCaretSettled(page)
