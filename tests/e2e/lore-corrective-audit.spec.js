...
test.describe('Whitespace-title validation and discard protection', () => {
  test('retains an invalid draft through the discard prompt\'s own Save retry, includes an unsubmitted tag on a real save, and protects link/tag-only edits', async ({ page }) => {
    ...
    // The forced click on a disabled record correctly did nothing (still on
    // New Entry, above) — that's the assertion this block exists for. The
    // discard-prompt Close/Cancel/Discard chain itself is already fully
    // exercised above (lines ~303-348), so rather than repeat it here (any
    // button whose own handler synchronously renders the "Save changes?"
    // prompt on top of itself — Close, or a freshly re-resolved Cancel
    // locator — was observed while writing this test to occasionally make
    // Playwright's post-click settle check treat the click as perpetually
    // intercepted/ambiguous, timing out even though the click was actually
    // delivered), just reload to discard this stray draft and reset state
    // cleanly — matching how a real navigation-away would drop it too.
    await page.reload()
    await waitForStorageHydration(page)
    await openLore(page)
